...
def deletar_funcionario(id):
    try:
        ...
        flash(f'Erro ao deletar o funcionário: {err}', 'error')
    finally:
        cursor.close()
        connection.close()
    return redirect(url_for('funcionarios'))
...
def deletar_setor(id):
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        cursor.execute("DELETE FROM setor WHERE id = %s", (id,))
        connection.commit()
        flash('Setor deletado com sucesso!', 'success')
    except mysql.connector.Error as err:
        flash(f'Erro ao deletar o setor: {err}', 'error')
    finally:
        cursor.close()
        connection.close()
    return redirect(url_for('setores'))
...
@app.route('/deletar_cargo/<int:id>', methods=['POST'])
def deletar_cargo(id):
    try:
        ...
        flash(f'Erro ao deletar o cargo: {err}', 'error')
    finally:
        cursor.close()
        connection.close()
    return redirect(url_for('cargos'))
